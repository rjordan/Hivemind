...
RSpec.describe ApplicationRecord, type: :model do
  ...
  describe 'UUID primary key configuration' do
    ...
    let(:user) { User.create!(name: 'app_rec_user', email: 'app_rec@example.com') }
  ...
  describe 'timestamp behavior' do
    let(:user) { User.create!(name: 'timestamp_user', email: 'timestamp@example.com') }
    ...
    it 'updates updated_at when record is modified' do
      ...
      user.update!(name: 'updated_username')
